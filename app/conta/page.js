...
export default function Account(){
 const[email,setEmail]=useState(''),[password,setPassword]=useState(''),[user,setUser]=useState(null),[msg,setMsg]=useState(''),[loading,setLoading]=useState(false),[discordLoading,setDiscordLoading]=useState(false),[site,setSite]=useState(defaultSite);
 useEffect(()=>{loadSite().then(setSite);api('/auth/me').then(d=>setUser(d.user)).catch(()=>{})},[]);
