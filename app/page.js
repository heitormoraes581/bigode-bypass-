...
export default function Home(){
  ...
  return <main style={style}>
    ...
    <section className="popular"><h2>{settings.popularTitle}</h2><div className="categoryList">
      {categories.filter(c=>c.active!==false).map(c=><button className={'category '+(category===c.slug?'selected':'')} key={c.id||c.slug} onClick={()=>{setCategory(c.slug);location.hash='produtos'}}>
    ...
    <footer><div><div className="brand foot"><img className="footerLogo" src={settings.logoUrl||'/logo-bigode-bypass.png'} alt={settings.siteName}/><strong>{settings.siteName}</strong></div><p>{settings.footerText}</p></div><div className="social"><a href={settings.discordUrl||'#'}>◉</a>　<a href={settings.youtubeUrl||'#'}>▶</a>　<a href={settings.tiktokUrl||'#'}>♪</a></div><div className="copy">{settings.copyright}</div><div className="links"><a href={settings.termsUrl||'#'}>{settings.termsLabel}</a>　 <a href={settings.privacyUrl||'#'}>{settings.privacyLabel}</a></div></footer>
